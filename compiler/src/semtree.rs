...
use crate::{
    ast::{
        self, Expr, ExternFunctionBody, ForDirection, ForLoop, FunctionBody, Loc, ProgramFile,
        Statement,
    },
...
impl SemanticTree {
    ...
    ) -> Result<Self, Vec<CompilerErrors>> {
    ...
    fn visit_program_file(&mut self, pf: &ProgramFile) -> Result<(), Vec<CompilerErrors>> {
    ...
    fn visit_function_decl(&mut self, func: &FunctionBody) -> Result<Function, CompilerErrors> {
        ...
        if let FunctionDecl::FunctionDecl { loc, input, output } = fd {
            ...
            for (i, (id, ty)) in function_args.iter().enumerate() {
                ...
                Self::insert_vardecl(
                    loc.clone(),
                    t,
                    STExpr::new(ty.clone(), loc.clone(), ExprKind::FunctionArg(i)),
                    &mut codeblock,
                    self.buildins.clone(),
                    true,
                )?
            ...
            if !return_arg.is_void() {
                Self::insert_vardecl(
                    loc.clone(),
                    result,
                    STExpr::new(return_arg.clone(), loc.clone(), ExprKind::Default),
                    &mut codeblock,
                    self.buildins.clone(),
                    false,
                )?;
    ...
    ) -> Result<ExternFunction, CompilerErrors> {
        let (_id, fd) = self
            .types_resolver
            .resolve_funccall(
                &func.loc,
    ...
    fn visit_codeblock(
        ...
        code_block
            .common_statements
            .push(STStatement::CodeBlock(loc, stmts));
        Ok(())
    }

    fn try_get_autoref(expr: STExpr) -> Result<STExpr, CompilerErrors> {
    ...
    fn autoref_or_pass(expr: STExpr) -> Result<STExpr, CompilerErrors> {
    ...
    fn insert_autoderef_or_pass(
        &mut self,
        expr: STExpr,
        force_no_clone: bool,
    ) -> Result<STExpr, CompilerErrors> {
        if let Some(ty) = expr.ret_type.get_underlying_autoderef_type() {
            ...
            if target.is_trivially_copiable() || force_no_clone {
                ...
                Ok(STExpr {
                    ...
                    loc: expr.loc.clone(),
                    kind: ExprKind::BuildInCall(BuildInCall {
                        func: buildin_clone?.unwrap(),
                        args: vec![expr],
                        ret_type: target,
                    }),
                })
    ...
    fn insert_impl_conversion(
        ...
        if from == to {
            ...
            if ty == to {
                ...
                return Err(CompilerErrors::ImplicitTypeConversionError(
                    loc,
                    ty.pretty_representation(),
                    to.pretty_representation(),
                ));
            }
    ...
    fn check_kind_of_function(
        ...
        fc: &ast::FunctionCall,
        scope: &Scope,
    ) -> Result<FunctionCallResolveResult, CompilerErrors> {
        let mut args = vec![];
        for a in &fc.args {
        ...
        if let ast::Expr::Ident(l, id) = fc.func.as_ref() {
            ...
            match t {
                Some((id, func)) => match func {
                    FunctionDecl::FunctionDecl {
                        ...
                        if args.len() != input.len() {
                            return Err(CompilerErrors::InvalidArgumentCount(
                                loc.clone(),
                                id.0.clone(),
                                args.len(),
                                input.len(),
                ...
                None => {
                    ...
                    if ty.is_err() {
                        return Err(CompilerErrors::UnknownFunctionOrTypename(
                            l.clone(),
                            id.to_string(),
                        ));
                    }
    ...
    fn resolve_typecast(
        ...
        let tck = if target == source {
            ...
            return Err(CompilerErrors::ExplicitTypeConversionError(
                loc.clone(),
                source.pretty_representation(),
                target.pretty_representation(),
            ));
        };
    ...
    fn insert_assignment(
        ...
        code_block: &mut CodeBlock,
        buildins: Arc<RefCell<BuildInModule>>,
        insert_autoclean: bool,
    ) -> Result<(), CompilerErrors> {
        let middle_drop = if target.required_type.is_trivially_copiable() && insert_autoclean {
            None
        } else {
            let drop = buildins
                .borrow_mut()
                .register_or_get_drop(&target.required_type)?
                .unwrap();
            let RhsKind::Deref(d) = &target.kind;
            Some(Box::new(STStatement::BuildInCall(
                loc.clone(),
                BuildInCall {
                    func: drop,
                    args: vec![d.clone()],
    ...
    fn visit_statement(
        ...
        match &statement {
            Statement::CodeBlock(l, b) => self.visit_codeblock(l.clone(), &b, outer, code_block),
            ...
            Statement::If(l, cond, mb, ab) => {
                ...
                let mbstmt = {
                    ...
                    Box::new(STStatement::CodeBlock(*l, cb))
                };
                let abstmt = {
                    if let Some(stmt) = ab {
                        let mut first_scope = Scope::new_with_outer(outer);
                        ...
                        Some(Box::new(STStatement::CodeBlock(*l, cb)))
                    } else {
                        None
                    }
                };
            ...
            Statement::While(l, cond, stmt) => {
                ...
                code_block
                    .common_statements
                    .push(STStatement::While(*l, Box::new(cond), mbstmt));
                Ok(())
            }
    ...
    fn visit_for_loop(
        ...
        if !from.ret_type.is_any_int() {
            return Err(CompilerErrors::IntExpected(
                from.loc.clone(),
                from.ret_type.pretty_representation(),
            ));
        }
        if !to.ret_type.is_any_int() {
            return Err(CompilerErrors::IntExpected(
                to.loc.clone(),
                to.ret_type.pretty_representation(),
            ));
        }
        if from.ret_type != to.ret_type {
            //TODO: Implement type hierarchy
            return Err(CompilerErrors::TypesNotSame(
                to.loc.clone(),
                from.ret_type.pretty_representation(),
                to.ret_type.pretty_representation(),
            ));
        ...
        let prim_ty = if let SLPType::PrimitiveType(pt) = ty.clone() {
            pt
        } else {
            todo!()
        };
        ...
        let var_name = if fl.is_new {
            ...
            if t.is_none() {
                return Err(CompilerErrors::LocalVariableDoesntExist(
                    to.loc.clone(),
                    name.0.clone(),
                ));
            }
        ...
        let change_expr = if let ForDirection::Up = fl.direction {
            STExpr::new(
                ...
                ExprKind::PrimitiveIntBinOp(
                    Box::new(var_expr.clone()),
                    Box::new(Self::build_int_constant(1, prim_ty, l.clone())?),
                    IntBinOp::Add,
                ),
            )
        } else {
        ...
        internal_codeblock
            .common_statements
            .push(STStatement::CodeBlock(l.clone(), for_body));
        Self::insert_assignment(
            l.clone(),
            target,
            change_expr,
            &mut internal_codeblock,
            self.buildins.clone(),
            true,
        )?;
        let stmt = STStatement::While(
            l.clone(),
            Box::new(control_expression),
            Box::new(STStatement::CodeBlock(l.clone(), internal_codeblock)),
        );
                ...
                STStatement::BuildInCall(
                    ...
                    BuildInCall {
                        ...
                        args: vec![STExpr::new(
                            ...
                            loc,
                            ExprKind::GetLocalVariableRef(variable_name.clone()),
                        )],
                        ret_type: SLPType::void(),
                    },
                ),
            )?;
            code_block.common_statements.push(STStatement::DeferHint(
                loc.clone(),
                code_block.defer_statements.len() - 1,
            ));
        }
        Ok(())
    }
    ...
    fn visit_int_constant(&self, cnst: &str, l: &Loc) -> Result<STExpr, CompilerErrors> {
        ...
        let ty = match rem {
            ...
            "u8" => STExpr {
                ...
                kind: ExprKind::NumberLiteral(NumberLiteral::U8({
                    ...
                    if !minus {
                        ...
                        return Err(CompilerErrors::NegateSymbolToUnsignedError(l.clone()));
                    }
                })),
            },
            "u16" => STExpr {
            ...
            a @ _ => return Err(CompilerErrors::InvalidSuffix(l.clone(), a.to_string())),
        };
        Ok(ty)
    }
    fn parse_int<'a>(
        ...
        digits: &'a str,
        base: u32,
    ) -> Result<(u64, &'a str), CompilerErrors> {
        let mut curr = 0;
        let mut acc = 0u64;
        for c in digits.chars() {
            ...
            if "01234567890abcdef".contains(c) {
                ...
                if digit >= base {
                    return Err(CompilerErrors::InvalidNumber(
                        loc.clone(),
                        digits.to_string(),
                    ));
                }
    ...
    fn visit_char_const(&self, cnst: &str, l: &Loc) -> Result<STExpr, CompilerErrors> {
    ...
    fn visit_indexation_expression(
        ...
        if index.ret_type.is_any_int() {
            ...
            Err(CompilerErrors::IntExpected(
                index.loc.clone(),
                index.ret_type.pretty_representation(),
            ))
        }
    }
        ...
        if let SLPType::AutoderefPointer(internal_ty) = &indexable_expr_ref.ret_type {
            ...
            } = internal_ty.as_ref()
            {
                ...
                if let SLPType::DynArray(da) = ty.as_ref() {
                    ...
                    let indexable_expr_ref = STExpr::new(
                        ty.wrap_autoderef_or_pass(),
                        loc.clone(),
                        ExprKind::GetElementBehindReffedReferenceCounter(Box::new(
                            indexable_expr_ref,
                        )),
                    );
                    let index_expr = self.visit_indexation_expression(index, 0, u64::MAX, scope)?;
                    let res = STExpr {
                        ret_type: ret_ty,
                        loc,
                        kind: ExprKind::GetElementRefInReffedArray(
                    ...
                    Err(CompilerErrors::ArrayExpected(
                        loc.clone(),
                        ty.pretty_representation(),
                    ))
                }
                ...
                Err(CompilerErrors::ArrayExpected(
                    loc.clone(),
                    internal_ty.pretty_representation(),
                ))
            }
        } else {
            unreachable!()
    ...
    fn visit_rhs_expression(
        ...
        expr: &Expr,
        scope: &Scope,
    ) -> Result<RhsExpr, CompilerErrors> {
        let expr = self.visit_expression(expr, scope)?;
        Self::to_rhs_expression(expr)
    }
    fn to_rhs_expression(expr: STExpr) -> Result<RhsExpr, CompilerErrors> {
        let try_take_autoref = Self::autoref_or_pass(expr)?;
        Ok(RhsExpr {
            required_type: try_take_autoref
                .ret_type
                .get_underlying_autoderef_type()
                .unwrap()
                .clone(),
    ...
    fn visit_expression(&mut self, expr: &Expr, scope: &Scope) -> Result<STExpr, CompilerErrors> {
        Ok(match expr {
            ...
            Expr::NilLiteral(l) => STExpr::new(
                SLPType::PrimitiveType(SLPPrimitiveType::Nil),
                l.clone(),
                ExprKind::NilLiteral,
            ),
        })
    ...
    fn visit_method_call(
        ...
        if let SLPType::RefCounter(internal) =
            autoderef.ret_type.get_underlying_autoderef_type().unwrap()
        {
            let expr = STExpr::new(
                internal.wrap_autoderef_or_pass(),
                l.clone(),
                ExprKind::GetElementBehindReffedReferenceCounter(Box::new(autoderef)),
            );
            self.generate_structure_method_call(l, field, expr)
        } else if let SLPType::Struct(_, _, _) =
            autoderef.ret_type.get_underlying_autoderef_type().unwrap()
        {
            self.generate_structure_method_call(l, field, autoderef)
        } else {
            Err(CompilerErrors::FieldDoesntExists(
                l.clone(),
                field.to_owned(),
                autoderef
                    .ret_type
    ...
    fn generate_structure_method_call(
        &mut self,
        l: &Loc,
        method_name: &str,
        reffed_struct_expr: STExpr,
    ) -> Result<STExpr, CompilerErrors> {
        if let SLPType::Struct(name, id, _) = &reffed_struct_expr
            .ret_type
            .get_underlying_autoderef_type()
            .unwrap()
        {
            ...
            let t = structdecl
                ...
                .iter()
                .enumerate()
                .find(|(_, a)| a.0 .0 == method_name);
            if let Some((index, (name, ty))) = t {
                println!("index: {index}, name: {}", name.0);
                Ok(STExpr {
                    ret_type: SLPType::AutoderefPointer(Box::new(ty.clone())),
                    loc: *l,
                    kind: ExprKind::GetElementRefInReffedRecord(
                        Box::new(reffed_struct_expr),
                        index as u32,
                    ),
                })
            } else {
        ...
        } else if let SLPType::DynArray(_dy) = &reffed_struct_expr
            .ret_type
            .get_underlying_autoderef_type()
            .unwrap()
        {
            if method_name == "Length" {
                Ok(STExpr::new(
                    SLPType::int32(),
                    l.clone(),
                    ExprKind::DynArrayIntLen(Box::new(STExpr::new(
                        reffed_struct_expr
                            .ret_type
                            .get_underlying_autoderef_type()
                            .unwrap()
                            .clone(),
                        l.clone(),
                        ExprKind::Deref(Box::new(reffed_struct_expr)),
                    ))),
                ))
                ...
                let high = STExpr::new(
                    ...
                    ExprKind::PrimitiveIntBinOp(
                        Box::new(len),
                        Box::new(Self::build_int_constant(
                            1,
                            SLPPrimitiveType::Int32,
                            l.clone(),
                        )?),
            ...
            .unwrap()
        {
            if method_name == "Length" {
                ...
                Self::build_int_constant(
                    ((*index_offset) as u64) + size - 1,
                    SLPPrimitiveType::Int32,
                    l.clone(),
                )
            } else {
    ...
    fn visit_new(
        ...
        if let SLPType::Struct(fid, id, _) = &t {
            ...
            if args_p.len() != st.fields.len() {
                return Err(CompilerErrors::InvalidNewOpArgumentCount(
                    loc.clone(),
                    format!("{}::{}", fid, id.0),
                    args_p.len(),
                    st.fields.len(),
            ...
            Ok(STExpr {
                ret_type: t,
                loc,
                kind: ExprKind::ConstructRecordFromArgList(reconst_exprs),
            })
            ...
            if let SLPType::Struct(fid, id, _) = rc.as_ref() {
                ...
                internal_content = STExpr {
                    ret_type: *rc.clone(),
                    loc,
                    kind: ExprKind::ConstructRecordFromArgList(reconst_exprs),
                };
            } else if let SLPType::DynArray(_da) = rc.as_ref() {
                if let Some(count_expr) = count {
                    ...
                    if !visited.ret_type.is_any_int() {
                        return Err(CompilerErrors::IntExpected(
                            loc.clone(),
                            visited.ret_type.pretty_representation(),
                        ));
                    }
                    ...
                    let id = self
                        .buildins
                        .borrow_mut()
                        .register_or_get_dyn_array_empty_constuctors(&rc)?;
                    internal_content = STExpr::new(
                        *rc.clone(),
                        loc.clone(),
                        ExprKind::BuildInCall(BuildInCall {
                            func: id,
                            args: vec![t],
                            ret_type: *rc.clone(),
                        }),
                ...
                return Err(CompilerErrors::InvalidTypeForNew(
                    loc.clone(),
                    t.pretty_representation(),
                ));
            }
            Ok(STExpr {
                ret_type: t.clone(),
                loc,
                kind: ExprKind::ConstructRefcounterFromInternalContent(Box::new(internal_content)),
            })
        } else {
            return Err(CompilerErrors::InvalidTypeForNew(
                loc.clone(),
    ...
    fn visit_int_bin_op(
        ...
        let le = Box::new(self.insert_autoderef_or_pass(vl, false)?);
        let vr = self.visit_expression(&r, scope)?;
        let re = Box::new(self.insert_autoderef_or_pass(vr, false)?);
        if le.ret_type == re.ret_type {
            if le.ret_type.is_any_int() {
                ...
                let bool_op = match kind {
                    ...
                    a @ _ => {
                        return Err(CompilerErrors::InvalidOperationForType(
                            loc.clone(),
                            match a {
                                IntBinOp::Add => "+",
                                IntBinOp::Substract => "-",
                            ...
                            .into(),
                            le.ret_type.pretty_representation(),
                        ));
                    }
                };
                Ok(STExpr {
            ...
            return Err(CompilerErrors::TypesNotSame(
                loc.clone(),
                le.ret_type.pretty_representation(),
                re.ret_type.pretty_representation(),
            ));
        }
    }
    fn visit_comparations(
        ...
        if vl.ret_type.is_nil() || vr.ret_type.is_nil() {
            let nilcheck = if vr.ret_type.is_nil() {
                Box::new(self.insert_autoderef_or_pass(vl, true)?)
            } else {
                Box::new(self.insert_autoderef_or_pass(vr, true)?)
            };
            if nilcheck.ret_type.is_nullable() {
                if let ComparationKind::Equal = kind {
                    return Ok(STExpr::new(
                        SLPType::bool(),
                        loc,
                        ExprKind::IsNull(nilcheck),
                    ));
                } else if let ComparationKind::NotEqual = kind {
                    let comp =
                        STExpr::new(SLPType::bool(), loc.clone(), ExprKind::IsNull(nilcheck));
                    return Ok(STExpr::new(
                        SLPType::bool(),
                        loc,
                        ExprKind::BoolUnaryOp(Box::new(comp), BoolUnaryOp::Not),
                    ));
                } else {
                    return Err(CompilerErrors::InvalidOperationForType(
                        loc.clone(),
                        ...
                        .into(),
                        SLPType::PrimitiveType(SLPPrimitiveType::Nil).pretty_representation(),
                    ));
                }
            } else {
                return Err(CompilerErrors::NotNullCheckableError(
                    loc,
                    nilcheck.ret_type.pretty_representation(),
                ));
            }
        }
        let le = Box::new(self.insert_autoderef_or_pass(vl, false)?);
    ...
    fn visit_int_unary_op(
        ...
        let inp = Box::new(self.insert_autoderef_or_pass(inp, false)?);
            ...
            let bool_op = match kind {
                ...
                _ => {
                    return Err(CompilerErrors::InvalidOperationForType(
                        ...
                        .into(),
                        inp.ret_type.pretty_representation(),
                    ));
                }
            };
            Ok(STExpr {
...
impl CodeBlock {
    pub fn new() -> Self {
        Self {
            common_statements: Default::default(),
            defer_statements: Default::default(),
        }
    }
}
#[derive(Debug, Clone)]
