...
pub fn function(func: &Function) -> StringTreeNode {
    StringTreeNode::with_child_nodes("function ".to_owned() + &func.function_name.0 + " (" + 
    &func.function_args.iter().map(|x|format!("{:?} {}", x.1, x.0.0)).reduce(|i1, i2|i1 + ", " + &i2).unwrap_or_default() + "): " + &format!("{:?}", func.return_arg),
...
pub fn statement(stmt: &STStatement) -> StringTreeNode {
    match stmt {
        ...
        STStatement::Print(_, e) => StringTreeNode::with_child_nodes("print".to_owned(), iter::once(expr(&e))),
