...
impl ContextSymbolResolver {
    ...
    pub fn resolve(&self, id: &Identificator) -> Result<Option<(Id, FunctionDecl)>, SemTreeBuildErrors> {
        if id.path.is_empty() {
...
impl<'a, 'b> TypeResolverGenerator<'a, 'b> {
    pub fn new(compiler: &'b Compiler) -> Self {
        Self { compiler, queue_to_resolve: Default::default(), next_round_resolve: Default::default(), resolved: Default::default() }
    }
        ...
        while self.queue_to_resolve.len() > 0 {
            for ((fid, id), ty) in &self.queue_to_resolve {
                if let Ok(t) = resolver.from_ast_type( ty, fid) {
                    resolver.internal.insert((fid.clone(), id.clone()), SLPTypeDecl::TypeAlias(t));
                }
                else {
                    self.next_round_resolve.insert((fid.clone(), id.clone()), &ty);
                }
            }
            mem::swap(&mut self.queue_to_resolve, &mut self.next_round_resolve);
            self.next_round_resolve.clear();
            if count <= self.queue_to_resolve.len() {panic!()}
