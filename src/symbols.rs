...
impl<'a, 'b> TypeResolverGenerator<'a, 'b> {
                ...
                for ty_decl in &section.decls {
                    match ty_decl {
                        crate::ast::TypeDeclElement::TypeAlias(_l, name, ty) => {
                            self.queue_to_resolve.insert(
                                (file, Id(name.clone())),
                                ResolveTarget::TypeAlias(_l.clone(), ty),
                            );
                        }
                        crate::ast::TypeDeclElement::RecordDeclare(_l, name, fields, rec_ty) => {
                            ...
                            for i in fields {
                                Rc::get_mut(&mut prr.field_order)
                                    .unwrap()
                                    .push(Id(i.id.clone()));
                                assert!(prr
                                    .to_be_resolved
                                    .insert(Id(i.id.clone()), &i.ty.ty)
                                    .is_none());
                            }

                            self.queue_to_resolve
                                .insert((file, Id(name.clone())), ResolveTarget::StructDecl(prr));
                        }
                    }
                }
            }
    ...
    pub fn resolve(mut self) -> Result<GlobalSymbolResolver, Vec<CompilerErrors>> {
                ...
                match rt {
                    ResolveTarget::TypeAlias(loc, ty) => {
                        match resolver.from_ast_type(&loc, ty, fid) {
                            ...
                            Err(err) => {
                                error_list.push(err);
                                self.next_round_resolve.insert(
                                    (fid.clone(), id.clone()),
                                    ResolveTarget::TypeAlias(loc.clone(), ty),
                                );
                            }
                        }
                    }
                    ResolveTarget::StructDecl(rs) => {
                        ...
                        let prr = PartialyResolvedRecord {
                            resolved: new_res,
                            to_be_resolved: new_to_be_resolved,
                            field_order,
                            is_class: rs.is_class,
                            loc: rs.loc.clone(),
                        };
                        if !prr.to_be_resolved.is_empty() {
                            self.next_round_resolve
                                .insert((fid.clone(), id.clone()), ResolveTarget::StructDecl(prr));
                        } else {
                            let mut fields = vec![];
                            ...
                            let st = StructType {
                                name: id.clone(),
                                fields,
                                is_class: prr.is_class,
                                is_copiable,
                            };
                            resolver
                                .types
                                .insert((fid.clone(), id.clone()), SLPTypeDecl::StructDecl(st));
                        }
                    }
                }
            }
...
impl GlobalSymbolResolver {
    pub fn get_fileid_checked(&self, loc: &Loc, name: &str) -> Result<FileId, CompilerErrors> {
        let target_fid = self.filename_translation.get(name);
        match target_fid {
            Some(f) => Ok(f.clone()),
            None => Err(CompilerErrors::UnknownFilename(
                loc.clone(),
                name.to_owned(),
            )),
        }
    }
    pub fn is_in_dep_list_checked(
        &self,
        loc: &Loc,
        checked: FileId,
        dep: FileId,
        ...
        if self.deps.get(&dep).unwrap().contains(&checked) {
            ...
            Err(CompilerErrors::FileIsNotInUses(loc.clone(), name))
        }
    }
    pub fn canonical_functions(&self, fid: &FileId, id: &Id) -> Id {
        ...
        if id.0 == "main" {
            ...
            match fn_type {
                FunctionDecl::FunctionDecl { .. } => Id(format!(
                    "{}${}",
                    self.reverse_filename_translation[fid], id.0
                )),
                FunctionDecl::ExternFunctionDecl { .. } => Id(format!("{}", id.0)),
            }
        }
    }
    pub fn fill_function_decl(
        &mut self,
        pf: &ProgramFile,
        fid: &FileId,
    ) -> Result<(), CompilerErrors> {
    ...
    fn convert_typedecls(
        &self,
        loc: &Loc,
        fid: &FileId,
        v: &[ArgDecl],
    ) -> Result<Vec<(Id, SLPType)>, CompilerErrors> {
        //v.iter()
    ...
    pub fn canonical_structures_name(&self, _fid: &FileId, _id: &Id) -> Id {
        todo!()
    }
    pub fn get_struct(&self, fid: &String, id: &Id) -> Result<Option<&StructType>, CompilerErrors> {
    ...
    pub fn resolve_funccall(
        &self,
        loc: &Loc,
        fid: &FileId,
        id: &Identificator,
    ) -> Result<Option<(Id, FunctionDecl)>, CompilerErrors> {
        if id.path.is_empty() {
            let id = Id(id.name.clone());
            if let Some(sym) = self.functions.get(&(fid.clone(), id.clone())) {
                return Ok(Some((self.canonical_functions(fid, &id), sym.clone())));
            }
            return Ok(None);
    ...
    pub fn from_ast_type(
        ...
        match ty {
            Type::Primitive(t) => {
                if t.path.is_empty() {
                        ...
                        a @ _ => {
                            ...
                            if let Some(ty) = self.types.get(&id) {
                                return match ty {
                                    ...
                                    SLPTypeDecl::StructDecl(d) => {
                                        if d.is_class {
                                            Ok(SLPType::RefCounter(Box::new(SLPType::Struct(
                                                fname.clone(),
                                                id.1,
                                                d.is_copiable,
                                            ))))
                                ...
                                return Err(CompilerErrors::BadType(loc.clone(), a.to_owned()));
                            }
                        }
                    }))
                    ...
                    if ty.is_none() {
                        ...
                        return Err(CompilerErrors::BadType(loc.clone(), t.to_string()));
                    }
                    let fname = self
                        .reverse_filename_translation
                        .as_ref()
            ...
            Type::Pointer(t) => Ok(SLPType::Pointer(Box::new(
                self.from_ast_type(loc, &t, file)?,
            ))),
            Type::DynArray(t) => Ok(SLPType::RefCounter(Box::new(SLPType::DynArray(Box::new(
                self.from_ast_type(loc, &t, file)?,
            ))))),
